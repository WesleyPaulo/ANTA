  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <div class="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
        ...
        <p class="mt-0.5 text-lg font-semibold">{{ hw()?.best_vram_gb ?? 0 }} <span class="text-sm font-normal text-slate-400">GB</span></p>
      </div>
      <div class="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
        <p class="text-xs uppercase tracking-wide text-slate-400">RAM</p>
        <p class="mt-0.5 text-lg font-semibold">{{ hw()?.ram_gb ?? 0 }} <span class="text-sm font-normal text-slate-400">GB</span></p>
      </div>
      <div class="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
        <p class="text-xs uppercase tracking-wide text-slate-400">Disco livre</p>
        <p class="mt-0.5 text-lg font-semibold">{{ hw()?.disk_free_gb ?? 0 }} <span class="text-sm font-normal text-slate-400">GB</span></p>
      </div>
      <div class="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
        ...
        <p class="mt-0.5 text-sm font-medium">{{ ambiente.label }}</p>
        ...
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-400 dark:bg-slate-800/50">
            <tr>
              ...
              <th class="px-4 py-1.5 font-medium">Modo</th>
              <th class="px-4 py-1.5 font-medium">VRAM mín</th>
              <th class="px-4 py-1.5 font-medium">Modelo</th>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            >
              ...
              <td class="px-4 py-1.5 tabular-nums">{{ m.vram_gb }} GB</td>
              <td class="px-4 py-1.5"><code class="text-xs">{{ m.llm }}</code></td>
              <td class="px-4 py-1.5"><StatusPill :status="m.status" /></td>
            </tr>
          </tbody>
        </table>
      </div>
