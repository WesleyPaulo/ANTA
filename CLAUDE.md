...
2. **O LLM nunca gera shell.** Ele so escolhe uma acao de `voz/actions/schema.py`
   via `instructor`. Todo efeito colateral vive em `voz/actions/executor.py`.
...
### 5. Executor — `voz/actions/executor.py`
...
### 6. Pipeline — `voz/core/pipeline.py`
- `run(audio)`: transcribe → decide → execute → retorna feedback.

### 7. Runtime / loop — `voz/__main__.py` (`run`)
...
### 8. Instalador — `voz/installer/app.py`
...
### 9. Atalho — `voz/platform/hotkey.py`
